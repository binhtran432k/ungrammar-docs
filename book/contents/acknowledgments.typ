...
#let advisor = if m.lecturers.len() > 1 [advisors] else [advisor]
#let advisorArticle = if m.lecturers.len() > 1 [their] else [his]
#let advisorArticleFirst = if m.lecturers.len() > 1 [Their] else [His]

= Acknowledgments

We would like to express our sincere gratitude to our #advisor, #m.lecturers.join([, ], last: [ and ]), for #advisorArticle invaluable guidance and support throughout this project.
